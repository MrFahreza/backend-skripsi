...
def import_from_excel(current_user_id):
    # ... (logika handle file dan validasi duplikasi tetap sama) ...
        ...
        for index, row in df.iterrows():
            ...
            operations.append(
                UpdateOne(
                    {"npm": npm},
                    {
                        "$set": {**doc, "updated_at": now}, # Selalu set updated_at
                        "$setOnInsert": {"created_at": now} # Hanya set created_at saat insert baru
                    },
                    upsert=True
                )
            )
